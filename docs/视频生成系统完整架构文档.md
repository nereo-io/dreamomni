...
7. **Webhook配置**: 设置回调URL接收状态更新

### 1.3 积分计算逻辑
```typescript
// config/video-models.ts:204-221
function calculateCredits(modelId: string, duration: number, hasAudio: boolean = false): number {
  ...
  // Veo3 模型支持音频，需要额外费用
  if (model.provider === VideoModelProvider.VEO3 && hasAudio && model.audioPremiumCredits) {
...
| Veo2 | Image to Video | 20 | 5s, 8s | 720p分辨率 |
...
### 2.3 状态更新来源
1. **Webhook被动更新**: `app/api/video-generation/webhook/route.ts`
...
### 4.3 存储策略
1. **双URL存储**:
   - `video_url_fal`: fal.ai原始URL（临时）
   - `video_url_r2`: R2存储URL（永久，优先使用）

2. **容错机制**:
   - R2上传失败时仍标记为 `COMPLETED`
   - 保留原始URL作为备用访问方式
...
const getProgressValue = () => {
  ...
  const elapsedSeconds = (Date.now() - new Date(generation.created_at).getTime()) / 1000;
...
const getFriendlyErrorMessage = (apiErrorMessage: string) => {
  ...
  if (statusCodeMatch) {
    ...
    switch (statusCode) {
      case 422: return "输入参数有问题，请检查设置后重试";
      case 500: return "服务器错误，请稍后重试";
      case 504: return "请求超时，请重试";
      default: return `发生错误 (${statusCode})，请重试`;
    }
  }
