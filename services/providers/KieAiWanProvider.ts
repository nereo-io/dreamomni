...
type WanAspectRatio = "16:9" | "9:16" | "1:1";
...
export class KieAiWanProvider implements VideoProvider {
  ...
  private normalizeAspectRatio(aspectRatio?: string): WanAspectRatio {
    const normalized = aspectRatio?.toString().trim().toLowerCase();
    if (normalized === "9:16") {
      return "9:16";
    }
    if (normalized === "1:1") {
  ...
  private resolveImageUrl(input: VideoGenerationRequest): string {
    ...
    throw new Error("Kie.ai Wan 2.5 image-to-video requires image_url");
  }

  private resolveEnablePromptExpansion(
    input: VideoGenerationRequest
  ): boolean | undefined {
    if (typeof input.enable_prompt_expansion === "boolean") {
      return input.enable_prompt_expansion;
    }
    if (typeof input.enable_prompt_enhancement === "boolean") {
      return input.enable_prompt_enhancement;
    }
    return undefined;
  }
  ...
  private extractVideoUrl(data: any): string | null {
    ...
    try {
      ...
      console.error("Failed to parse Wan resultJson:", error);
    }
  ...
  async submit(
    ...
    try {
      ...
      const providerModelId =
        modelConfig?.providerModelId || input.model || "wan/2-5-text-to-video";
      const isImageModel = isImageToVideoModel(model);
      ...
      if (isImageModel) {
        requestBody.input.image_url = this.resolveImageUrl(input);
      } else {
        requestBody.input.aspect_ratio = this.normalizeAspectRatio(
          input.aspect_ratio
        );
      }
      ...
      const enablePromptExpansion = this.resolveEnablePromptExpansion(input);
      if (typeof enablePromptExpansion === "boolean") {
        requestBody.input.enable_prompt_expansion = enablePromptExpansion;
      }
      ...
      throw new Error(
        `Kie.ai Wan Provider submit failed: ${
  ...
  async status(
    ...
    try {
      ...
      throw new Error(
        `Kie.ai Wan status check failed: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  async result(
    ...
    try {
      ...
      throw new Error(
        `Kie.ai Wan result retrieval failed: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
