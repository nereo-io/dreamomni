...
export default async function ({ params }: { params: { uuid: string } }) {
  ...
  //根据params.uuid获取chatSession
  const chatSession = await getChatSessionByUuid(params.uuid);
  if (!chatSession) {
    return <Empty message="chat session not found" />;
  }
  ...
  // 基于user_uuid获取membership
  const membership = await findMembershipByUserUuid(chatSession.user_uuid);

  // 结构化信息展示组件
  const InfoDisplay = () => (
    <div className="p-4 mb-4 bg-gray-200 rounded-lg shadow-md">
      <h2 className="text-lg font-bold">聊天会话信息</h2>
      <p>
        <strong>聊天会话 UUID:</strong> {chatSession.uuid}, {chatSession.status}
        ,
        {moment(chatSession.created_at).format("YYYY-MM-DD HH:mm:ss") ||
          "未知时间"}
      </p>
      <p>
      ...
      <p>
        <strong>会员状态:</strong> {membership?.status || "No"},{" "}
        {moment(membership?.created_at).format("YYYY-MM-DD") || "未知时间"}
      </p>
    </div>
  );
